<template>
	<div class="login-wrap ">
		<h1>用户登录</h1>
		<el-form status-icon ref="ruleForm" label-width="100px" class="login-container">
			<el-form-item label="用户名" prop="pass">
				<el-input type="text" v-model="userName" autocomplete="off" placeholder="帐号"></el-input>
			</el-form-item>
			<el-form-item label="密码" prop="checkPass" >
				<el-input type="password"  v-model="userPwd" autocomplete="off" placeholder="密码"></el-input>
			</el-form-item>
			<el-form-item>
...
<script>
	...
	export default {
		...
		methods:{
			dosubmit:function(){
				// let url="http://localhost:8080/vue_test/userMsg/userAction!login";
				//var me=this;
        ...
        this.$router.push("/home");
				...
				/* this.axios.post(url,params).then((resp)=>{
					...
					if (resp.data.success) {
						...
						this.$router.push("/home");
						//用vuex保存登录的用户
						this.$store.commit('setUser',{user:resp.data.data});
						//获取客户营销机会开发成功&失败数量
						this.getChance();
				...
				/*
				//提交时使用转换之后的参数
				axios.post(url,paramsStr).then(function(response){
			...
			/* getChance:function(){
				let url=this.axios.urls.SALPLAN_LOOK_STATUS;

				//获取开发成功的数量
				this.axios.post(url,{chc_status:3}).then(resp=>{
					this.$store.commit('setSuccessd',{successd:resp.data.data})
				}).catch(resp=>{

				})

				//获取开发失败的数量
				this.axios.post(url,{chc_status:4}).then(resp=>{
					this.$store.commit('setDefeated',{defeated:resp.data.data})
				}).catch(resp=>{
			...
			/* getServiceStatus:function(){
				let url=this.axios.urls.SERVICE_MANAGE_STATUS;


				this.axios.post(url,{svrStatus:'新创建'}).then(resp=>{
					this.$store.commit('setServiceCreate',{serviceCreate:resp.data.data})
				}).catch(resp=>{

				})


				this.axios.post(url,{svrStatus:'已分配'}).then(resp=>{
					this.$store.commit('setServiceAllocation',{serviceAllocation:resp.data.data})
				}).catch(resp=>{

				})


				this.axios.post(url,{svrStatus:'已处理'}).then(resp=>{
					this.$store.commit('setServiceDispose',{serviceDispose:resp.data.data})
				}).catch(resp=>{

				})


				this.axios.post(url,{svrStatus:'已归档'}).then(resp=>{
					this.$store.commit('setServiceArchive',{serviceArchive:resp.data.data})
				}).catch(resp=>{
